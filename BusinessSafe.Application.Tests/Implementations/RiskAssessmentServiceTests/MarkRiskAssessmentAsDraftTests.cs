...
namespace BusinessSafe.Application.Tests.Implementations.RiskAssessmentServiceTests
{
    [TestFixture]
    [Category("Unit")]
    public class MarkRiskAssessmentAsDraftTests
    {
        ...
        [Test]
        public void Given_valid_request_When_mark_for_draft_Then_calls_the_correct_methods()
        {
            //Given
            var riskAssessmentService = CreateRiskAssessmentService();

            var request = new MarkRiskAssessmentAsDraftRequest()
                              {
                                  CompanyId = 100, 
                                  UserId = Guid.NewGuid(), 
            ...
            //When
            riskAssessmentService.MarkRiskAssessmentAsDraft(request);

            //Then
            _riskAssessmentRepository.Verify(x => x.SaveOrUpdate(mockRiskAssessment.Object));
            mockRiskAssessment.Verify(x => x.MarkAsDraft(user));
        }
