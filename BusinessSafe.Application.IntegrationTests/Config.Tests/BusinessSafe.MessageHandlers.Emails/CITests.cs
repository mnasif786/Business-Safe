...
namespace BusinessSafe.Application.IntegrationTests.Config.Tests.BusinessSafe.MessageHandlers.Emails
{
    ...
    public class CITests : BaseConfigTests
    {
        ...
        [TestCase("BusinessSafe.Messages.Events.EmployeeChecklistEmailGenerated, BusinessSafe.Messages",
            "businesssafe.messagehandlers")]
        [TestCase("BusinessSafe.Messages.Events.EmployeeChecklistCompleted, BusinessSafe.Messages",
            "businesssafe.messagehandlers")]
        [TestCase("BusinessSafe.Messages.Events.TaskAssigned, BusinessSafe.Messages",
            "businesssafe.messagehandlers.emails")]
        [TestCase("BusinessSafe.Messages.Events.ReviewAssigned, BusinessSafe.Messages",
            "businesssafe.messagehandlers.emails")]
        ...
        [TestCase("DBSubscriptionStorageConfig", "connection.connection_string",
            @"Initial Catalog=NServiceBus;Data Source=localhost\sql2008r2;UID=intranetadmin;PWD=intadpas;Connect Timeout=20;"
            )]
        [TestCase("TimeoutPersisterConfig", "connection.connection_string",
            @"Initial Catalog=NServiceBus;Data Source=localhost\sql2008r2;UID=intranetadmin;PWD=intadpas;Connect Timeout=20;"
            )]
        [TestCase("NHibernateSagaPersisterConfig", "connection.connection_string",
            @"Initial Catalog=NServiceBus;Data Source=localhost\sql2008r2;UID=intranetadmin;PWD=intadpas;Connect Timeout=20;"
            )]
        public void Then_NHibernateProperties_are_as_expected(string containingElementName, string keyAttributeValue,
            string value)
        {
            // Given
            var elements = _target
        ...
        private XDocument GetTarget()
        {
            ...
            switch (System.Environment.MachineName.ToLower())
            {
                case "pbs43758":
                    transformedFilePath =
                        @"D:\Code\BusinessSafe\BusinessSafe.MessageHandlers.Emails\App.CI.Transformed.config";
                    break;
                default:
                    transformedFilePath =
                        @"C:\BusinessSafe\Artifacts\BusinessSafe.MessageHandlers.Emails\Configs\App.CI.Transformed.config";
                    break;
            }
