...
{
    public class GetTaskDueTomorrowQuery : IGetTaskDueTomorrowQuery
    {
        public IList<Task> Execute(ISession session)
        {
            return session
                 ...
                 .Add(Restrictions.Eq("SendTaskDueTomorrowNotification", true))
                 .Add(Subqueries.PropertyNotIn("Id",
                     DetachedCriteria.For<EscalationTaskDueTomorrow>()
                         .SetProjection(Projections.Property("TaskId"))
                         .Add(Restrictions.IsNotNull("TaskDueTomorrowEmailSentDate"))
                     ))
